...
#-----------------------------------------------------------------------------------------------------#  
  ...
  cyw <- join(cyw,dat.y[,c('yr','spGDD.st','spGDD.st2','spPCP.st','spPCP.st2')],by='yr',type='left')
...
#GCMs
  mod <- data.frame(model=1:6,modelname=sort(unique(springclim$modelname)))
...
#For comparison, first calculate the expected means in each county and year between 2004-2018
  ...
  Xcounty <- as.matrix(cbind(rep(1,nrow(cyw.obs)),                                     
                             cyw.obs[,c('spGDD','spGDD2','spPCP','spPCP2',
                                        'avgGDD','diffGDD','diffGDD2','diffavgGDD',
                                        'avgPCP','diffPCP','diffPCP2','diffavgPCP',
                                        'gly','crop','glycrop')]))
...
#Summarizing forecasts for a low emissions scenario, SSP126 (across GCMs)
  ...
  #Summarize period means across 6000 iteration-GCM combinations
...
#Summarize forecasts for each emission scenario (across GCMs)
  #Summarize period means (mean, sd, quants) across (6000) iteration-GCM combinations for each scenario
...
#First remove big objects
  rm(preds.annmax,preds.annmd,preds.annmin,preds.annmn,preds.annsd,preds.bycp1,preds.bycp4,
     preds.bycpm1,preds.bycpm4,c.obs.byperiod,c.obs.list,obs.log,preds.byperiod,preds.byperiod2,
     preds.list.cpm1,preds.list.cpm4,preds.list.s,preds.low,split.cy,split.muc,split.mucy)  
